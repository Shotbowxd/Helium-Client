...
public class Slider extends Component {
	...
	public void renderComponent() {
		...
		clientFont.drawStringWithShadow(this.set.getName() + ": " + this.set.getValDouble() , (parent.parent.getX() + 2), (parent.parent.getY() + offset + 2) , -1);
	...
	public void updateComponent(int mouseX, int mouseY) {
		...
		renderWidth = this.parent.parent.getWidth() * (set.getValDouble() - min) / (max - min);
		...
		if (dragging) {
			if (diff == 0) {
				...
				double newValue = roundToPlace(((diff / this.parent.parent.getWidth()) * (max - min) + min), 2);
