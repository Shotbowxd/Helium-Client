...
import rip.helium.event.events.EventStoppable;
import rip.helium.event.types.Priority;

/**
 * 
 * 
 * @author DarkMagician6, Michael Shipley
...
public final class EventManager {
    ...
    private static void register(Method method, Object object) {
    	...
    	if (REGISTRY_MAP.containsKey(indexClass)) {
    		if (!REGISTRY_MAP.get(indexClass).contains(data))
    			REGISTRY_MAP.get(indexClass).add(data); sortListValue(indexClass);
    	} else {
     ...
     *
     * @see rip.helium.event.EventTarget
     */
