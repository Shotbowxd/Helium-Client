...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn)
    {
        ...
        {
            int k = Minecraft.getMinecraft().fontRendererObj.getStringWidth(this.getPlayerName(networkplayerinfo));
            i = Math.max(i, k);

            if (scoreObjectiveIn != null && scoreObjectiveIn.getRenderType() != IScoreObjectiveCriteria.EnumRenderType.HEARTS)
            {
                k = Minecraft.getMinecraft().fontRendererObj.getStringWidth(" " + scoreboardIn.getValueFromObjective(networkplayerinfo.getGameProfile().getName(), scoreObjectiveIn).getScorePoints());
                j = Math.max(j, k);
            }
        }
        ...
        if (this.header != null)
        {
            ...
            for (String s : list1)
            {
                l1 = Math.max(l1, Minecraft.getMinecraft().fontRendererObj.getStringWidth(s));
            }
        }
        ...
        if (this.footer != null)
        {
            ...
            for (String s2 : list2)
            {
                l1 = Math.max(l1, Minecraft.getMinecraft().fontRendererObj.getStringWidth(s2));
            }
        }

        if (list1 != null)
        {
            drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list1.size() * 10, Integer.MIN_VALUE);

            for (String s3 : list1)
            {
                int i2 = Minecraft.getMinecraft().fontRendererObj.getStringWidth(s3);
                Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(s3, (float)(width / 2 - i2 / 2), (float)k1, -1);
                k1 += 10;
            }
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            if (k4 < list.size())
            {
                ...
                if (networkplayerinfo1.getGameType() == WorldSettings.GameType.SPECTATOR)
                {
                    ...
                    Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(s1, (float)j2, (float)k2, -1862270977);
                }
                else
                {
                    Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(s1, (float)j2, (float)k2, -1);
                }
        ...
        {
            k1 = k1 + i4 * 9 + 1;
            drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list2.size() * 10, Integer.MIN_VALUE);

            for (String s4 : list2)
            {
                int j5 = Minecraft.getMinecraft().fontRendererObj.getStringWidth(s4);
                Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(s4, (float)(width / 2 - j5 / 2), (float)k1, -1);
                k1 += 10;
            }
        }
    }
