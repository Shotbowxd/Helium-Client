...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        //TODO: Client
        UpdateEvent event = new UpdateEvent(this.rotationYaw, this.rotationPitch, this.posY, this.onGround);
        ...
        if (mc.gameSettings.thirdPersonView != 0) {
            setServerSidePitch(event.getPitch());
            setServerSideYaw(event.getYaw());
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, event.getPosY(), this.posZ, event.getYaw(), event.getPitch(), event.isOnground()));
                }
                else if (flag2)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, event.getPosY(), this.posZ, event.isOnground()));
                }
                else if (flag3)
                {
